...
response = create_text_to_cad.sync(
    ...
    body=TextToCadCreateBody(
        prompt="Design a gear with 15 teeth",
    ),
)
...
if result.status == ApiCallStatus.FAILED:
    print(f"Da operation has failed: {result.error}")
    ...
    print(f"Operation has been completed and returned {len(result.outputs)} files:")
